...
PK['led-strip'] = {
    ...
    letterMaker: function(letter) {
        
        let html = '<div class="led-letter">';
        for (let i = 0; i < 8; i++) {
            html += '<div class="led-row">';
            
            const leds = PK['led-strip'].letters[letter][i].split('');
    ...
    syntheticData: function(synDataType) {
        ...
        if (synDataType === 'numbers') {
            ...
            function generateSynNumbers(num) {
            ...
            function generate() {
                ...
                chart.streamTo(PK['led-strip'].canvas, 250);
            }
    ...
    drawMessage: function(msg) {
        ...
        for (let i = 0; i < this.stripContent.length; i++) {
            html += PK['led-strip'].letterMaker(this.stripContent[i]);
        }
    ...
    init: function() {
        ...
        const stopSynData = function(event) {
            ...
            for (let i = 0, j = PK['led-strip'].synDataType.length; i < j; i++) {
                PK['led-strip'].synDataType[i].disabled = false;
            }

            PK['led-strip'].synDataGenerator.removeEventListener('click', stopSynData);
            PK['led-strip'].synDataGenerator.addEventListener('click', startSynData, false);
            PK['led-strip'].synDataGenerator.innerText = 'start';
            PK['led-strip'].synDataGenerator.classList.remove('button-primary');

            event.preventDefault();
        };

        const startSynData = function(event) {
            
            for (const val of PK['led-strip'].synDataType) {
                if (val.checked) {
                    
                    for (let i = 0, j = PK['led-strip'].synDataType.length; i < j; i++) {
                        PK['led-strip'].synDataType[i].disabled = true;
                    }

                    PK['led-strip'].synDataGenerator.innerText = 'stop';
                    PK['led-strip'].synDataGenerator.classList.add('button-primary');
                    PK['led-strip'].synDataGenerator.removeEventListener('click', startSynData);
                    PK['led-strip'].synDataGenerator.addEventListener('click', stopSynData, false);
